...
export async function setupDatabase() {
  ...
  // Single source of truth for user-authored queries (Query Builder). Dashboards
  // and rules/alerts reference a saved query by id instead of carrying their own SQL.
  ...
  // Type 3: Action controls (read-only, blocked tools, allowed HTTP methods)
  ...
  // Type 4: Usage limits (per-role token/request budgets)
  ...
  // Type 5: Content filtering (input topic blocklist/allowlist)
  ...
  // Type 6: Egress audit log (data leaving the network)
  ...
  // Type 7: Human-in-the-loop pending confirmations
  ...
  // Guardrail settings: hitl enabled, injection detection, egress logging
  ...
  // Seed defaults
  await sql`INSERT OR IGNORE INTO guardrail_settings (key, value) VALUES
    ('hitl_enabled', 'false'),
  ...
  // Personal edition: register a "Mosaic Files" data source PER top-level subfolder of
  // ~/Mosaic/files (mounted at /mosaic-files), plus a root source for loose files. This
  // gives users real folder boundaries — organised subfolders become separate sources
  // the AI treats independently. Runs at boot; the scheduler refreshes it periodically
  // as folders are added/removed. See lib/mosaic-folders.ts.
  if ((process.env.MOSAIC_EDITION || 'personal') === 'personal' && process.env.MOSAIC_FILES_DIR) {
    try {
      const { syncMosaicFolders } = await import('./mosaic-folders')
