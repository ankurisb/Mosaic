...
import { queryRaw, isPostgres } from '@/lib/db'
...
export async function GET(req: Request) {
  ...
  // Boolean literal differs by dialect (SQLite stores 0/1, Postgres true/false).
  const trueLit = isPostgres() ? 'true' : '1'
  ...
  if (isRca === '1') { conditions.push(`is_rca = ${trueLit}`) }
  if (q) {
  ...
  const entries = rows.slice(0, limit).map(r => ({
    ...
    is_rca:          r.is_rca === 1 || r.is_rca === true,
    web_search_used: r.web_search_used === 1 || r.web_search_used === true,
  }))
