...
export async function GET() {
  ...
  const [
    ...
    sql`SELECT COUNT(*) as cnt FROM sso_config WHERE enabled = true`.catch(() => [{ cnt: 0 }]),
    sql`SELECT COUNT(*) as cnt FROM guardrail_data_access WHERE enabled = true`.catch(() => [{ cnt: 0 }]),
    sql`SELECT COUNT(*) as cnt FROM guardrail_actions WHERE enabled = true`.catch(() => [{ cnt: 0 }]),
    sql`SELECT COUNT(*) as cnt FROM guardrail_usage_limits WHERE enabled = true`.catch(() => [{ cnt: 0 }]),
    sql`SELECT COUNT(*) as cnt FROM guardrail_content WHERE enabled = true`.catch(() => [{ cnt: 0 }]),
