...
const SUGGESTIONS = [
  ...
  'What databases and APIs do I have connected?',
  'Explain what you can help me with',
  'Write a short Python script to sort a list',
]
...
export default function ChatPage({ user }: { user: SessionUser }) {
  ...
  async function send(text: string) {
    ...
    } catch (err) { updateLast(cid, m => ({ ...m, content: '⚠ ' + (err instanceof Error ? err.message : 'Something went wrong') })) }
  ...
  const isStreaming = (i: number) => streaming && i === active.messages.length - 1
    ...
    <div style={{ display: 'flex', height: '100vh', overflow: 'hidden' }}>
      ...
      <div style={{ width: 220, flexShrink: 0, display: 'flex', flexDirection: 'column', background: 'var(--bg2)', borderRight: '1px solid var(--border)' }}>
        ...
        <div style={{ flex: 1, overflowY: 'auto', padding: 6 }}>
          {convs.map(c => (
            <div key={c.id} onClick={() => setActiveId(c.id)} style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', padding: '7px 8px', borderRadius: 5, cursor: 'pointer', marginBottom: 2, border: `1px solid ${c.id === activeId ? 'var(--border2)' : 'transparent'}`, background: c.id === activeId ? 'var(--bg3)' : 'none', fontSize: 11, color: c.id === activeId ? 'var(--text)' : 'var(--text2)' }}>
              <span style={{ overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap', flex: 1 }}>{c.title}</span>
              <button onClick={e => delConv(c.id, e)} style={{ background: 'none', border: 'none', cursor: 'pointer', color: 'var(--text3)', fontSize: 14, paddingLeft: 4 }}>×</button>
            </div>
          ))}
        </div>
        <div style={{ borderTop: '1px solid var(--border)', padding: '8px 10px', display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
          <button onClick={() => router.push('/settings')} style={{ background: 'none', border: 'none', fontSize: 11, color: 'var(--text3)', cursor: 'pointer', padding: '4px 6px', borderRadius: 4 }}>⚙ Settings</button>
          <div style={{ position: 'relative' }}>
            <div onClick={() => setShowUserMenu(v => !v)} style={{ width: 28, height: 28, borderRadius: '50%', background: 'var(--bg3)', border: '1px solid var(--border2)', display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: 11, fontWeight: 500, cursor: 'pointer' }}>
              {user.name.slice(0, 2).toUpperCase()}
            </div>
            {showUserMenu && (
              <>
                <div onClick={() => setShowUserMenu(false)} style={{ position: 'fixed', inset: 0, zIndex: 40 }} />
                <div style={{ position: 'absolute', bottom: 'calc(100% + 6px)', right: 0, background: 'var(--bg2)', border: '1px solid var(--border2)', borderRadius: 8, padding: 4, width: 190, zIndex: 50 }}>
                  ...
                  <button onClick={() => { setShowUserMenu(false); router.push('/settings') }} style={{ width: '100%', padding: '7px 10px', background: 'none', border: 'none', fontSize: 11, color: 'var(--text2)', cursor: 'pointer', textAlign: 'left', borderRadius: 4 }}>Settings</button>
                  <button onClick={signOut} style={{ width: '100%', padding: '7px 10px', background: 'none', border: 'none', fontSize: 11, color: 'var(--text2)', cursor: 'pointer', textAlign: 'left', borderRadius: 4 }}>Sign out</button>
      ...
      <div style={{ flex: 1, display: 'flex', flexDirection: 'column', overflow: 'hidden' }}>
        ...
        <div style={{ borderBottom: '1px solid var(--border)', flexShrink: 0 }}>
          <button onClick={() => setShowSystem(v => !v)} style={{ width: '100%', background: 'none', border: 'none', padding: '7px 16px', fontSize: 10, color: 'var(--text3)', cursor: 'pointer', textAlign: 'left', display: 'flex', alignItems: 'center', gap: 5, letterSpacing: '.06em' }}>
        ...
        <div style={{ flex: 1, overflowY: 'auto', padding: '24px 32px' }}>
          {active.messages.length === 0 ? (
            <div style={{ paddingTop: 60 }}>
              ...
              <div style={{ fontSize: 11, color: 'var(--text3)', marginBottom: 28 }}>Powered by Claude · Web search · DB & API queries · ugx.ai</div>
              <div style={{ display: 'flex', flexWrap: 'wrap', gap: 8 }}>
                {SUGGESTIONS.map(s => (
                  <button key={s} onClick={() => send(s)} style={{ padding: '7px 14px', border: '1px solid var(--border2)', borderRadius: 20, background: 'var(--bg3)', fontSize: 11, color: 'var(--text2)', cursor: 'pointer' }}>{s}</button>
                ))}
              </div>
            </div>
          ) : (
            active.messages.map((msg, i) => (
              <div key={i} style={{ marginBottom: 24 }}>
                <div style={{ fontSize: 9, color: 'var(--text3)', textTransform: 'uppercase', letterSpacing: '.12em', marginBottom: 5 }}>{msg.role === 'user' ? 'you' : 'claude'}</div>
                {msg.tools?.map((tc, j) => (
                  <div key={j} style={{ background: 'var(--bg3)', borderLeft: '2px solid var(--border2)', borderRadius: '0 6px 6px 0', padding: '8px 12px', marginBottom: 6, fontSize: 11 }}>
                    ...
                    {tc.result !== undefined && (
                      <div style={{ color: 'var(--gt)', marginTop: 3, fontSize: 10 }}>✓ {JSON.stringify(tc.result).slice(0, 160)}{JSON.stringify(tc.result).length > 160 ? '…' : ''}</div>
                    )}
                  </div>
                ))}
                <div style={{ fontSize: 13, lineHeight: 1.8, whiteSpace: 'pre-wrap', color: msg.role === 'user' ? 'var(--text2)' : 'var(--text)' }}>
                  {msg.content}
                  {msg.role === 'assistant' && isStreaming(i) && (
                ...
                {msg.role === 'assistant' && i === active.messages.length - 1 && !streaming && msg.content && (
                  <div style={{ display: 'flex', flexWrap: 'wrap', gap: 5, marginTop: 12 }}>
                    {['Go deeper', 'Simplify this', 'Give examples', 'What next?'].map(label => (
                      <button key={label} onClick={() => send(label)} style={{ padding: '4px 11px', border: '1px solid var(--border2)', borderRadius: 14, background: 'none', fontSize: 10, color: 'var(--text3)', cursor: 'pointer' }}>{label}</button>
                    ))}
                  </div>
                )}
              </div>
        ...
        <div style={{ borderTop: '1px solid var(--border)', padding: '12px 20px 18px', flexShrink: 0 }}>
          <div style={{ display: 'flex', gap: 8, alignItems: 'flex-end' }}>
            <textarea value={input}
              ...
              placeholder="Message… (Enter to send, Shift+Enter for new line)" rows={1}
              style={{ flex: 1, resize: 'none', background: 'var(--bg3)', border: '1px solid var(--border2)', borderRadius: 8, padding: '9px 13px', fontSize: 13, color: 'var(--text)', outline: 'none', lineHeight: 1.6 }} />
            <button onClick={() => send(input)} disabled={streaming || !input.trim()}
              style={{ flexShrink: 0, padding: '9px 18px', borderRadius: 8, border: 'none', background: streaming || !input.trim() ? 'var(--border2)' : 'var(--text)', color: 'var(--bg)', fontSize: 13, cursor: streaming || !input.trim() ? 'not-allowed' : 'pointer', fontWeight: 500 }}>
              {streaming ? '…' : 'Send'}
            </button>
          </div>
        </div>
