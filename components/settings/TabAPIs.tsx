...
export default function TabAPIs({ user }: { user: SessionUser }) {
  ...
  const authFields: Record<string, React.ReactNode> = {
    ...
    oauth2_client: <Grid cols={2}><Field label="Client ID"><input style={INP} value={svcForm.client_id} onChange={e => setSvc('client_id', e.target.value)} /></Field><Field label="Client secret"><input style={{ ...INP, fontFamily: 'var(--font-mono)', fontSize: 12 }} type="password" value={svcForm.client_secret} onChange={e => setSvc('client_secret', e.target.value)} /></Field><Field label="Token URL" hint="e.g. https://api.hubapi.com/oauth/v1/token"><input style={INP} placeholder="https://..." value={svcForm.token_url} onChange={e => setSvc('token_url', e.target.value)} /></Field><Field label="Refresh token (optional)" hint="For refresh_token grant flow (Zoho, Salesforce). Leave blank for client_credentials."><input style={{ ...INP, fontFamily: 'var(--font-mono)', fontSize: 12 }} type="password" placeholder="1000.xxx..." value={svcForm.refresh_token} onChange={e => setSvc('refresh_token', e.target.value)} /></Field><Field label="Auth header prefix" hint='Default "Bearer". Use "Zoho-oauthtoken" for Zoho APIs.'><input style={INP} placeholder="Bearer" value={svcForm.header_prefix} onChange={e => setSvc('header_prefix', e.target.value)} /></Field><Field label="API version (optional)"><input style={INP} placeholder="e.g. 2024-11-20" value={svcForm.api_version} onChange={e => setSvc('api_version', e.target.value)} /></Field></Grid>,
